...
{
    Task<IEnumerable<StudentProgress>> GetAllStudentProgressAsync(); // получение данных всех прогрессов студентов
    Task<StudentProgress> GetStudentProgressByIdAsync(int id); // получение данных прогресса студента по id
    Task<List<StudentProgress>> GetStudentProgressByStudentIdAsync(int id); // получение данных прогресса студента по id студента
